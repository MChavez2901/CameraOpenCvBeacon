...
public class MatFilter {
    ...
    private static final String
            ...
            sobel_tag = "Sobel", laplacian_tag = "Laplacian", canny_tag = "Canny", hough_tag = "Hough", custom_tag = "CustomColors";

    Mat mSrcMat; // Mat to use as the base to run filters on
    String mTag = "Original"; // string denoting desired filter to run
   ...
   public static Mat getCanny(Mat srcMat, int lowerThreshold, int upperTheshold) {
    ...
    private static void findButton(int x1, int x2, int y1, int y2, String Color1, Bitmap bitmap) {
        ...
        if (Math.abs(blackyVal*blackxVal) > 0) {
            ...
            printVLine(buttonx, 1, bitmap);
            printHLine(buttony, 1, bitmap);
        }
    }
    private static void printVLine(int xVal, int color, Bitmap bitmap) {
